			...
			<div className='flex items-center justify-center gap-10 2xl:gap-20 w-full flex-col-reverse md:flex-row'>
				<div className='w-full h-[632px] overflow-hidden rounded-xl'>
					<Image
						...
						height={1000}
						src={app1}
						className='object-cover object-center rounded-xl xl:h-[632px]  hover:scale-105 transition-all ease-in-out duration-300'></Image>
				</div>
				<div className='xl:p-10 px-5 py-10 text-slate-50 bg-gradient-to-br from-blue-600 to-blue-950 w-full h-full rounded-xl flex  flex-col gap-5'>
					<h3 className='font-semibold text-3xl xl:text-4xl md:text-balance capitalize'>
						ABBMA + Medicar
					</h3>
					<p className='font-light text-sm md:text-base mb-5'>
					...
					<ul className='flex flex-col gap-5 w-full'>
						{associate.map((item, index) => {
							return (
								<li
									key={index}
									className='flex gap-2 text-sm items-center w-full text-left'>
									<CircleCheckBig className='flex-shrink-0 w-5 h-5 ' />
									{item}
								</li>
							);
						})}
